...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        # Polarization-modulated adaptive gate (Suggestion 1): input = content (dim) +
        # pol_stat (1) + pol_std (1).  pol_std quantifies intra-window ice heterogeneity,
        # directly encoding the physical reason to suppress or amplify logit residual reuse.
        # Zero-init by _reset_pa2_weights → sigmoid(0)=0.5 at init → neutral residual blend.
        self.alpha_gate = nn.Sequential(
            nn.Linear(dim + 2, num_heads),   # +2: pol_stat, pol_std
            nn.Sigmoid()
        )
    ...
    def forward(self, x, mask=None, attn_residual=None, pol_feat=None):
        ...
        # ② Polarization-modulated adaptive logit gate (Suggestion 1).
        # pol_std captures intra-window ice heterogeneity: high std (ice-edge windows)
        # suppresses logit reuse; low std (homogeneous water/ice) allows more reuse.
        if attn_residual is not None:
            if self.use_adaptive_gate:
                x_stat = x.mean(dim=1)                             # (B_, C)
                if pol_feat is not None:
                    pol_stat = pol_feat.mean(dim=1)                # (B_, 1)
                    pol_std  = pol_feat.std(dim=1)                 # (B_, 1)
                else:
                    pol_stat = torch.zeros(B_, 1, device=x.device, dtype=x.dtype)
                    pol_std  = torch.zeros(B_, 1, device=x.device, dtype=x.dtype)
                joint = torch.cat([x_stat, pol_stat, pol_std], dim=-1)  # (B_, C+2)
                gate = self.alpha_gate(joint)                      # (B_, num_heads)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 use_adaptive_gate=True, use_pol_bias=True, kimi_history_window=0):
        super().__init__()
        self.dim = dim
        ...
        self.use_pol_bias = use_pol_bias
        self.kimi_history_window = kimi_history_window
    ...
    def _kimi_retrieve(self, history_blocks, partial_block, pseudo_query):
        ...
        """
        if self.kimi_history_window > 0:
            history_blocks = history_blocks[-self.kimi_history_window:]
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 kimi_history_window=kimi_history_window)
...
class GlobalContextInjection(nn.Module):
    ...
    def __init__(self, local_dim: int = 96, global_dim: int = 768, num_heads: int = 4):
        ...
        # Polarization-guided query (Suggestion 2): local tokens with atypical
        # polarization signatures generate different queries, routing them toward
        # relevant global context more effectively.  Zero-init → no effect at start.
        self.pol_embed = nn.Linear(1, local_dim, bias=False)

    def forward(self, local_feat: torch.Tensor, global_feat: torch.Tensor,
                pol_map: torch.Tensor = None) -> torch.Tensor:
        """
        local_feat:  (B, L_local,  local_dim)  — Stage-0 tokens, e.g. (B, 256, 96)
        global_feat: (B, L_global, global_dim) — Stage-3 tokens, e.g. (B,   4, 768)
        pol_map:     (B, L_local,  1) or None  — Stage-0 polarization ratio per token
        ...
        # Polarization-guided query: pol_embed shifts the query in proportion to each
        # token's polarization signature, so ice-edge tokens query differently than
        # homogeneous water/ice tokens (Suggestion 2).
        norm_local = self.norm_local(local_feat)
        if pol_map is not None:
            norm_local = norm_local + self.pol_embed(pol_map)  # (B, L, local_dim)
        Q = self.proj_q(norm_local)                            # (B, L, local_dim)
...
class SegformerMLPDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(embed_dim * n_scales, embed_dim,
        ...
        # Polarization-guided scale attention (Suggestion 3): pol ratio at each pixel
        # determines how much each scale contributes to the fused representation.
        # Zero-init → softmax([0,...,0]) = 1/n → scale_mod = 1.0 at init,
        # so this is a no-op initially (identical to standard SegFormer concat).
        self.pol_scale_attn = nn.Conv2d(1, n_scales, kernel_size=1, bias=False)

    def forward(self, features, target_hw, pol_map=None):
        """
        ...
        pol_map:   (B, L0, 1) Stage-0 polarization ratio map, or None
        ...
        if pol_map is not None:
            # Polarization-guided scale attention (Suggestion 3).
            # scale_mod starts at 1.0 (zero-init → uniform softmax → *n_scales = 1.0),
            # then learns to emphasize scales that best match each pixel's ice type.
            B_p, L0, _ = pol_map.shape
            H0 = int(L0 ** 0.5)
            pol_2d  = pol_map.transpose(1, 2).view(B_p, 1, H0, H0)
            pol_cHW = F.interpolate(pol_2d, size=(common_H, common_W),
                                    mode='bilinear', align_corners=False)
            scale_w   = self.pol_scale_attn(pol_cHW).softmax(dim=1)         # (B, n, cH, cW)
            scale_mod = (scale_w * self._n_scales).unsqueeze(2)             # (B, n, 1, cH, cW)
            weighted  = [projected[i] * scale_mod[:, i] for i in range(self._n_scales)]
            x = torch.cat(weighted, dim=1)                                   # (B, C*n, cH, cW)
        else:
...
class CORALHead(nn.Module):
    """Independent K-1 sigmoid classifiers (soft CORAL / ordinal output head).

    Each threshold k gets its own Conv2d projection from embed_dim, avoiding the
    gradient-collapse failure of the original shared-weight design (where the majority
    water class drove the shared projection to zero → P(y>k)→0 for all k).

    Output is P(y > k) for k=0,...,K-2, encoded as sigmoid probabilities in (0, 1).
    Use coral_cdf_to_probs() in functions.py to convert to K class probs for inference.
    """

    def __init__(self, embed_dim: int, num_classes: int):
        super().__init__()
        self.conv = nn.Conv2d(embed_dim, num_classes - 1, kernel_size=1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """x: (B, embed_dim, H, W)  →  (B, K-1, H, W)  P(y > k) ∈ (0, 1)"""
        return torch.sigmoid(self.conv(x))


class UNetDecoder(nn.Module):
    """U-Net style decoder with skip connections for Swin Transformer features.

    Takes the same 4-scale token sequences as SegformerMLPDecoder and
    progressively upsamples with DoubleConv + skip connections.
    Total ~2.1M parameters (vs ~0.63M for MLP/PG-Decoder).
    """

    def __init__(self, in_channels=(96, 192, 384, 768), num_classes=5, bilinear=True):
        super().__init__()
        # in_channels order: [ft0, ft1, ft2, ft4] (shallow→deep)
        c0, c1, c2, c4 = in_channels
        self.up1 = Up(c4 + c2, c2, bilinear)          # 768+384=1152 → 384, 4×4
        self.up2 = Up(c2 + c1, c1, bilinear)          # 384+192=576  → 192, 8×8
        self.up3 = Up(c1 + c0, c0, bilinear)          # 192+96=288   → 96, 16×16
        self.up4 = Up(c0, c0 // 2, bilinear)           # 96 → 48, 32×32
        self.up5 = Up(c0 // 2, c0 // 4, bilinear)      # 48 → 24, 64×64
        self.up6 = Up(c0 // 4, c0 // 8, bilinear)      # 24 → 12, 128×128
        self.out_conv = nn.Conv2d(c0 // 8, num_classes, kernel_size=1)

    def forward(self, features, target_hw):
        """features: list of (B, L_i, C_i); target_hw: (H, W)."""
        feats_2d = []
        for f in features:
            B, L, C = f.shape
            Hf = int(L ** 0.5)
            feats_2d.append(f.transpose(1, 2).view(B, C, Hf, Hf))

        f0, f1, f2, f4 = feats_2d  # shallow→deep: (96,16×16), (192,8×8), (384,4×4), (768,2×2)

        x = self.up1(f4, f2)        # (B, 384, 4, 4)
        x = self.up2(x, f1)         # (B, 192, 8, 8)
        x = self.up3(x, f0)         # (B,  96, 16, 16)
        x = self.up4.up(x)
        x = self.up4.conv(x)        # (B,  48, 32, 32)
        x = self.up5.up(x)
        x = self.up5.conv(x)        # (B,  24, 64, 64)
        x = self.up6.up(x)
        x = self.up6.conv(x)        # (B,  12, 128, 128)
        # Final bilinear to exact target size if needed
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, options, **kwargs):
        ...
        self._pa2_pgci   = options.get('pa2_pgci',          True)
        self._pa2_pg_decoder = options.get('pa2_pg_decoder', True)
        self._kimi_history_window = options.get('kimi_history_window', 0)
        self.decoder_type = options.get('decoder_type', 'pg')  # 'unet', 'mlp', 'pg'
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               kimi_history_window=self._kimi_history_window)
            self.layers.append(layer)

 
        # ── Decoder: GlobalContextInjection + Decoder ──────
        ...
        in_ch = (embed_dim, embed_dim * 2, embed_dim * 4, self.num_features)
        ...
        # Optionally replace the decoder head with a CORAL ordinal head.
        # CORALHead outputs K-1 sigmoid CDF values instead of K softmax logits.
        if options.get('sod_head') == 'coral':
            self.decoder.head = CORALHead(embed_dim=256, num_classes=options['n_classes']['SOD'])
    ...
    def _reset_pa2_weights(self):
        ...
        # GCI: zero-init output proj → no-op at init; gate already zeros
        nn.init.zeros_(self.gci.proj_out.weight)
        # P-GCI: zero-init pol_embed → no polarization shift at init (Suggestion 2)
        nn.init.zeros_(self.gci.pol_embed.weight)
        # PG-Decoder: zero-init pol_scale_attn (only for 'pg' decoder type)
        if self.decoder_type == 'pg':
            nn.init.zeros_(self.decoder.pol_scale_attn.weight)
    ...
    def forward_features(self, x):
        _need_pol = self._pa2_pol or self._pa2_pgci or self._pa2_pg_decoder
        if self.pol_ratio_channel and _need_pol:
            pol_raw = x[:, -1:, :, :]                                        # (B, 1, H, W)
            ...
            pol_map_stage0 = pol_map                                         # saved for P-GCI & PG-Decoder
        else:
            pol_map = None
            pol_map_stage0 = None
        ...
        return ft, pol_map_stage0



    def forward(self, x):
        H, W = x.shape[2], x.shape[3]
        ft, pol_map_stage0 = self.forward_features(x)
        ...
        # P-GCI: polarization-guided query generation + global context injection
        ft0 = self.gci(ft[0], ft[4], pol_map=pol_map_stage0 if self._pa2_pgci else None)

        # Decoder: route based on decoder_type
        if self.decoder_type == 'unet':
            ...
            sod = self.decoder([ft0, ft[1], ft[2], ft[4]], target_hw=(H, W),
                               pol_map=pol_map_stage0 if self._pa2_pg_decoder else None)
