...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # Attention Residual: per-head learnable gate, zero-initialized so the
        # model starts as vanilla Swin and learns when/how much to use the residual.
        self.alpha = nn.Parameter(torch.zeros(num_heads))

    def forward(self, x, mask=None, attn_residual=None):
        """
        Args:
        ...
        # Apply SW-MSA cyclic-shift mask (must come before attn_residual so that
        # masked positions remain suppressed even after the residual is added).
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N, N)

        # Attention Residual: add scaled logits from the previous same-type block.
        # alpha is broadcast over (batch*nW, heads, N, N).
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_residual=None):
        """
        Args:
            x: input token sequence, shape (B, H*W, C)
            attn_residual: pre-softmax attention logits from the previous same-type
                           block, shape (nW*B, num_heads, N, N) or None.
        Returns:
            x: output token sequence, shape (B, H*W, C)
            pre_softmax_attn: attention logits before softmax (nW*B, num_heads, N, N),
                              to be forwarded to the next same-type block.
        """
        H, W = self.input_resolution
        B, L, C = x.shape
        ...
        # W-MSA/SW-MSA (with optional attention residual)
        attn_windows, pre_softmax_attn = self.attn(
            x_windows, mask=self.attn_mask, attn_residual=attn_residual
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        # Two independent residual streams, one per block type.
        # Even-indexed blocks (i % 2 == 0) are W-MSA  → attn_res_wmsa
        # Odd-indexed  blocks (i % 2 == 1) are SW-MSA → attn_res_swmsa
        # Each stream carries the pre-softmax attention logits from block l
        # to block l+2 of the same type, implementing A_l += alpha * A_{l-2}.
        attn_res_wmsa = None
        attn_res_swmsa = None
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.use_checkpoint:
                # checkpoint.checkpoint passes only positional tensor args; attn_residual
                # defaults to None inside blk, so residual propagation is disabled
                # in checkpoint mode (safe fallback, use_checkpoint=False in this project).
                result = checkpoint.checkpoint(blk, x)
            else:
                result = blk(x, attn_residual=current_residual)

            x, pre_softmax_attn = result

            # Update the appropriate residual stream for the next same-type block.
            if i % 2 == 0:
