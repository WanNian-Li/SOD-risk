'''
推理专用的 z-score 归一化脚本，用于 4b_pack_for_inference.py 产出的 NC。

与训练版 5_apply_zscore_from_stats_wa.py 的差异：
1. NC_ROOT / OUTPUT_ROOT 指向推理数据目录
2. STATS_JSON 必须复用训练时的统计量，保证推理输入分布与模型训练分布一致
3. 输入 NC 已将陆地像素设为 -9999（来自 4b_pack_for_inference.py），
   因此 global_valid_mask 会自动把陆地视为 invalid，
   无需在此脚本做任何额外的陆地感知逻辑
4. coastline_land_mask 变量会被原样透传（不归一化，走 else 分支）

其余实现与训练版完全一致，直接复用训练统计量。
'''
...
# 输入：4b_pack_for_inference.py 的输出目录
NC_ROOT = r"F:/ZJU/11_Ice/dataset_create/data_apply/data_nc_inference"
# 输出：供 inference.py 读取的归一化目录
OUTPUT_ROOT = r"F:/ZJU/11_Ice/dataset_create/data_apply/data_nc_inference_zscore"
# 统计来源：必须复用训练时的 zscore_stats.json，不得重算
